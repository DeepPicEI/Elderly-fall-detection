...
class Config:
    ...
    # 视频配置（相对于脚本所在目录，需手动将视频放入指定文件夹）
    VIDEO_FOLDER = 'ceshipin'  # 视频所在文件夹名称
    VIDEO_NAME = 'Screenrecording_20250318_152051.mp4'  # 视频文件名
    VIDEO_PATH = os.path.join(BASE_DIR, VIDEO_FOLDER, VIDEO_NAME)

    # 保存路径配置（自动创建结果文件夹）
    SAVE_FOLDER = os.path.join(BASE_DIR, 'results1')
    ...
    INFERENCE_PARAMS = {
        ...
        'save': False,
        'show': False,
        'project': os.path.join(BASE_DIR, 'runs/predict'),  # 相对路径
        'name': 'exp',
        'save_txt': False,
        'save_conf': True,
        'save_crop': False,
